...
exports.updateWatchlist = function(quantity,price,user_id,diamond_id,callback){
    return new Promises(function(resolve,reject){
        var update_query = " UPDATE `tbl_watchlist` SET `quantity`=?, `sub_total`=? WHERE `user_id`=? AND `diamond_id`=?";
...
exports.updateDiamondStock = function(diamond_id,callback) {   
    return new Promises(function(resolve,reject){
        var update_query =" UPDATE `tbl_admin_diamond` SET stock_quantity=stock_quantity-1 WHERE diamond_id = ?";
        connection.query(update_query,[diamond_id],function(err,result){
            err ? reject(err) : resolve(result);            
          });
    });
}
